...
def ecc_same_add(G):#自身加自身
    ...
    x3 = (k * k - x1 - x1) % p
    y3 = (k * (x1 - x3) - y1) % p
    return [zero_fill(hex(x3)[2:],64), zero_fill(hex(y3)[2:],64)]
def ecc_diff_add(G1, G2):#不同坐标点相加
    x1 = chr_to_int(G1[0])
    y1 = chr_to_int(G1[1])
    x2 = chr_to_int(G2[0])
    y2 = chr_to_int(G2[1])
    ...
    y3 = (k * (x1 - x3) - y1) % p
    return [zero_fill(hex(x3)[2:],64), zero_fill(hex(y3)[2:],64)]
def ecc_diff_sub(G1,G2):  #减法
    x1 = chr_to_int(G1[0])
    y1 = chr_to_int(G1[1])
    x2 = chr_to_int(G2[0])
    p = chr_to_int(ecc[-1])
    y2 = (-1*chr_to_int(G2[1])) % p
    tmp1 = (y2 - y1) % p
    tmp2 = findModInverse((x2 - x1) % p, p)
    k = tmp1 * tmp2 % p
    x3 = (k * k - x1 - x2) % p
    y3 = (k * (x1 - x3) - y1) % p
    return [zero_fill(hex(x3)[2:],64), zero_fill(hex(y3)[2:],64)]
def ecc_diff_add_near(pointbase, G1):#和比自己大1的点相加
    return ecc_diff_add(pointbase, ecc_same_add(G1))
def ecc_multiply(k, G):#椭圆曲线乘法
    #print(G)
    k1=chr_to_int(k)
    if k1 == 2:
        return ecc_same_add(G)
    if k1 == 3:
        return ecc_diff_add(G,ecc_same_add(G))
    if k1 % 2 == 0:
        return ecc_same_add( ecc_multiply(hex(k1 // 2)[2:],G))
    if k1 % 2 == 1:
        return ecc_diff_add_near(G,ecc_multiply(hex(k1 // 2)[2:],G))

def SM2_encrypt(plaintext):
    klen=len(plaintext)*4
    k=hex(random.randint(1,n-1))[2:]
    #k = "4C62EEFD6ECFC2B95B92FD6C3D9575148AFA17425546D49018E5388D49DD7B4F"  #该k值为官方文档给出的实例,应用时取上面的随机数
    pB=ecc_multiply(dB,G)
    C1=ecc_multiply(k,G)
    temp=ecc_multiply(k,pB)
...
def ecc_point_true(ecc,point):
    ...
    if (y*y)%p == (x*x*x +a*x +b )%p:
        ...
        return False
def SM2_decrypt(cipher,klen):
    clen=len(cipher)
    C1=cipher[:130]
    C1=[C1[2:66],C1[66:]]
    if not ecc_point_true(ecc,C1):
        return False
    temp=ecc_multiply(dB,C1)
    t=KDF(temp[0]+temp[1],klen,v)
    #print("t:",t)
    if chr_to_int(t)==0:
        return False
...
# klen=len(M)*4
# cipher=SM2_encrypt(M)
# print(SM2_decrypt(cipher, klen))
